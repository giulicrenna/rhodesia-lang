...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider('rhodesia', autocompleteProvider, '.', ' '));
    // ----------------------------------------------------------------
    // Hover provider — shows signature and documentation
    // Handles both plain names (sqrt) and module-qualified (math.sqrt)
    // ----------------------------------------------------------------
    context.subscriptions.push(vscode.languages.registerHoverProvider('rhodesia', {
        provideHover(document, position, token) {
            ...
            if (moduleDescriptors[word]) {
                ...
                if (funcCount > 0)
                    md.appendMarkdown(`${funcCount} function${funcCount !== 1 ? 's' : ''}`);
                if (constCount > 0)
                    md.appendMarkdown(`, ${constCount} constant${constCount !== 1 ? 's' : ''}`);
    ...
    // ----------------------------------------------------------------
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider('rhodesia', {
        provideDocumentSymbols(document, token) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (funMatch) {
                    ...
                    symbols.push(new vscode.DocumentSymbol(funcName, 'Function', vscode.SymbolKind.Function, range, range));
                }
                // Variables: type: name
                ...
                if (varMatch) {
                    ...
                    symbols.push(new vscode.DocumentSymbol(varName, `Variable (${varMatch[1]})`, vscode.SymbolKind.Variable, range, range));
                }
            }
            return symbols;
        }
    ...
    // ----------------------------------------------------------------
    context.subscriptions.push(vscode.languages.registerSignatureHelpProvider('rhodesia', {
    ...
    // ----------------------------------------------------------------
    context.subscriptions.push(vscode.languages.registerDefinitionProvider('rhodesia', {
    ...
    // Info command
    context.subscriptions.push(vscode.commands.registerCommand('rhodesia.showInfo', () => {
        vscode.window.showInformationMessage('Rhodesia Language Extension\n\n' +
