        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = 0; j < line.length; j++) {
                if (line[j] === '{') {
                    ...
                    braceDepth--;
                    if (braceDepth < 0) {
                        diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, j, i, j + 1), 'Unexpected closing brace', vscode.DiagnosticSeverity.Error));
                        braceDepth = 0;
                    }
                }
            }
            ...
            if (funDeclMatch && !line.includes('->')) {
                ...
                diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, funIdx, i, funIdx + 3 + 1 + funDeclMatch[1].length), `Function '${funDeclMatch[1]}' is missing a return type (-> type)`, vscode.DiagnosticSeverity.Warning));
            }
        }
        // Report unclosed braces at end of file
        if (braceDepth > 0) {
            const lastLine = document.lineCount - 1;
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(lastLine, 0, lastLine, 0), `Missing ${braceDepth} closing brace${braceDepth > 1 ? 's' : ''}`, vscode.DiagnosticSeverity.Error));
        }
    }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (intAssignMatch) {
                ...
                if (/\d+\.\d+/.test(value)) {
                    ...
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, matchIdx, i, line.length), 'Cannot assign float value to int variable', vscode.DiagnosticSeverity.Warning));
                }
            }
        }
    }
